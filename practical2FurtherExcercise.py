...
def scale_bar(ax, location=(0.92, 0.95)):
    ...
    ax.plot([sbx, sbx - 10000], [sby, sby], color='k', linewidth=9, transform=ax.projection) # plot a thick black line, 10 km long
    ax.plot([sbx, sbx - 5000], [sby, sby], color='k', linewidth=6, transform=ax.projection) # plot a smaller black line from 0 to 5 km long
    ax.plot([sbx-5000, sbx - 10000], [sby, sby], color='w', linewidth=6, transform=ax.projection) # plot a white line from 5 to 10 km
    ax.plot([sbx - 1000, sbx - 1000], [sby - 1000, sby + 1000], color='k', linewidth=6, transform=ax.projection) # plot a smaller black line from 0 to 1 km long

    ax.text(sbx, sby-2000, '10 km', transform=ax.projection, fontsize=8) # add a label at 10 km
    ax.text(sbx-6000, sby-2000, '5 km', transform=ax.projection, fontsize=8) # add a label at 5 km
    ax.text(sbx-11000, sby-2000, '1 km', transform=ax.projection, fontsize=8) # add a label at 1 km
...
for ii, name in enumerate(county_names):
    feat = ShapelyFeature(counties.loc[counties['CountyName'] == name, 'geometry'], # first argument is the geometry
                          ...
                          facecolor=county_colors[ii % len(county_colors)], # set the face color to the corresponding color from the list
...
# Plot towns and cities with different markers
towns_subset = towns[towns['STATUS'] == 'Town']
cities_subset = towns[towns['STATUS'] == 'City']

town_handle = ax.plot(towns_subset.geometry.x, towns_subset.geometry.y, 's', color='0.5', ms=6, transform=ccrs.PlateCarree(), label='Towns')
city_handle = ax.plot(cities_subset.geometry.x, cities_subset.geometry.y, '^', color='red', ms=8, transform=ccrs.PlateCarree(), label='Cities')
...
handles = county_handles + water_handle + river_handle + [town_handle[0], city_handle[0]] # use '+' to concatenate (combine) lists
